...
exports.matchValue = function (valueGetter, message) {
    ...
    if (!message) {
        message = '%s does not match expected value: "%s"';
    }
    return function (form, field, callback) {
...
exports.requiresFieldIfEmpty = function (alternate_field, message) {
    if (!message) {
        message = 'One of %s or %s is required.';
    }
...
exports.minlength = function (val, message) {
    if (!message) {
        message = 'Please enter at least %s characters.';
    }
    return function (form, field, callback) {
...
exports.maxlength = function (val, message) {
    if (!message) {
        message = 'Please enter no more than %s characters.';
    }
    return function (form, field, callback) {
        if (field.data.length <= val) {
            callback();
...
exports.color = function (message) {
    if (!message) {
        message = 'Inputs of type "color" require hex notation, e.g. #FFF or #ABC123.';
    }
...
exports.date = function (message) {
    if (!message) {
        message = 'Inputs of type "date" must be valid dates in the format "yyyy-mm-dd"';
    }
    var numberRegex = /^[0-9]+$/,
        invalidDate = new Date('z');
    return function (form, field, callback) {
        var parts = field.data ? field.data.split('-') : [];
        var allNumbers = parts.every(function (part) {
