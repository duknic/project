...
<table id="source">
    <tbody>
    <tr>
        <td><h1>node-memcached</h1></td>
        <td></td>
    </tr>
    <tr class="filename">
        <td><h2 id="lib/memcached.js"><a href="#">memcached</a></h2></td>
        <td>lib/memcached.js</td>
    </tr>
    <tr class="code">
        <td class="docs">
...
<pre><code>// generate a regular query,
    ...
    // fetch servers
    server = server ? server : redundancy &amp;&amp; queryRedundancy ? (redundancy =
    this.HashRing.createRange(query.key, (this.redundancy + 1), true)).shift() : this.HashRing.getNode(query.key);

    // check if the server is still alive
    if (server in this.issues &amp;&amp; this.issues[server].failed) return query.callback &amp;&amp;
    query.callback(false, false);
    ...
    if (S.readyState !== 'open') return query.callback &amp;&amp; query.callback('Connection readyState is set to ' +
    S.readySate);
...
<pre><code>var issues
    ...
    issues.log(error);</code></pre>
            ...
            <p> // These do not need to be publicly available as it's one of the most important
                ...
                , 'CLIENT<em>ERROR': function(tokens, dataSet, err){ err.push(tokens.splice(1).join(' ')); return
                    [CONTINUE, false] }
                    , 'SERVER</em>ERROR': function(tokens, dataSet, err, queue, S, memcached){
                memcached.connectionIssue(tokens.splice(1).join(' '), S); return [CONTINUE, false] }</p>
            ...
            <p> , 'STORED': function(tokens, dataSet){ return [CONTINUE, true] }
                ...
                , 'END': function(tokens, dataSet, err, queue){ if (!queue.length) queue.push(false); return [FLUSH,
                true] }</p>
...
<pre><code> // add references to the retrieved server
    ...
    return response;
            ...
            <p> // When working with large chunks of responses, node chunks it in to pieces. So we might have
                ...
                // in to one response stream. With no indication where it had cut the data. So it can be it cuts inside
                the value response,
                // or even right in the middle of a line-break, so we need to make sure, the last piece in the buffer is
                a LINEBREAK
...
<pre><code>if (Buffer.isBuffer(value)){
    ...
    if (length &gt; memcached.maxValue) return private.errorResponse('The length of the value is greater than ' +
    memcached.maxValue, callback);
    ...
    LINEBREAK + value
            ...
            <p> // Curry the function and so we can tell the type our private set function
                memcached.set = Utils.curry(false, private.setters, 'set', [['key', String], ['lifetime', Number],
                ['value', String], ['callback', Function]]);
                memcached.replace = Utils.curry(false, private.setters, 'replace', [['key', String], ['lifetime',
                Number], ['value', String], ['callback', Function]]);
                memcached.add = Utils.curry(false, private.setters, 'add', [['key', String], ['lifetime', Number],
                ['value', String], ['callback', Function]]);</p>

            <p> memcached.cas = function checkandset(key, value, cas, lifetime, callback){
                private.setters.call(this, 'cas', [['key', String], ['lifetime', Number], ['value', String],
                ['callback', Function]], key, value, lifetime, callback, cas);
                };</p>

            <p> memcached.append = function append(key, value, callback){
                private.setters.call(this, 'append', [['key', String], ['lifetime', Number], ['value', String],
                ['callback', Function]], key, value, 0, callback);
                };</p>

            <p> memcached.prepend = function prepend(key, value, callback){
                private.setters.call(this, 'prepend', [['key', String], ['lifetime', Number], ['value', String],
                ['callback', Function]], key, value, 0, callback);
                };</p>
...
<pre><code> // handle multiple servers
    ...
    <tr class="filename">
        <td><h2 id="lib/utils.js"><a href="#">utils</a></h2></td>
        <td>lib/utils.js</td>
    </tr>
    <tr class="code">
        <td class="docs">
...
<pre><code>switch(tokens[1]){
    ...
    <tr class="filename">
        <td><h2 id="lib/connection.js"><a href="#">connection</a></h2></td>
        <td>lib/connection.js</td>
    </tr>
    <tr class="code">
        <td class="docs">
            <p>var EventEmitter = require('events').EventEmitter
...
<pre><code>issue.emit('reconnected', issue.details);
    ...
    Utils.merge(issue, JSON.parse(JSON.stringify(issue.config)));</code></pre>
            ...
            <p>Manager.isAvailable = function(connection){
                ...
                return (readyState == 'open' || readyState == 'writeOnly') &amp;&amp; !(connection.<em>writeQueue &amp;&amp;
                    connection.</em>writeQueue.length);
                };</p>
            ...
            <p> if (connection.readyState &amp;&amp; connection.readyState !== 'closed' &amp;&amp; connection.end)
                connection.end();
                };</p>
